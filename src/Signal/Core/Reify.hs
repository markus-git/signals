...
data Key (i :: (* -> *) -> * -> *) (a :: *) where
  Key :: Witness i a => Name (S Symbol i a) -> Key i a
...
freify f =
  let (_, graph) = let a = Sig (S.variable (toDyn f)) in (a, f a)
   in reify graph
...
reify' :: forall i a. Witness i a => Symbol i a -> Reify i (Key i a)
...
insertNode :: Witness i a => Ref (S Symbol i a) -> Node i (S Symbol i a) -> Reify i (Key i a)
...
lookupName :: Witness i a => Ref (S Symbol i a) -> Reify i (Maybe (Key i a))
